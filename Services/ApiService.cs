...
namespace GeoTrackerApp3.Services
{
    public static class ApiService
    {
        ...
        /// <summary>
        /// Fetches a paged list of people for the admin member list screen.
        /// Mirrors GET /api/Member/GetAdminMemberList.
        /// </summary>
        public static async Task<MemberListResponse> GetAdminMemberListAsync(
            int memberId,
            string sortBy = "",
            ...
            string token = "")
        {
            try
            {
                ...
                var query =
                    ...
                    $"&filterBy={Uri.EscapeDataString(filterBy ?? string.Empty)}" +
                    $"&filterValue={Uri.EscapeDataString(filterValue ?? string.Empty)}" +
                    $"&pageNumber={pageNumber}" +
                    $"&pageSize={pageSize}";

                var url = $"/api/Member/GetAdminMemberList?{query}";
                ...
                if (response.IsSuccessStatusCode)
                {
                    var result = await response.Content.ReadFromJsonAsync(
                        AppJsonContext.Default.MemberListResponse, cancellationToken: cts.Token);
                    return result ?? new MemberListResponse();
                }

                var msg = await TryGetErrorMessage(response);
                ErrorDisplayService.ShowError("People", msg ?? $"Server returned {(int)response.StatusCode}");
                return new MemberListResponse();
            }
            catch (TaskCanceledException)
            {
                ErrorDisplayService.ShowError("People", "Request timed out while loading people.");
                return new MemberListResponse();
            }
            catch (Exception ex)
            {
        ...
        /// <summary>
        /// Fetches full detail for a single person to populate the edit form.
        /// Mirrors GET /api/Member/Get?id={memberId}.
        /// </summary>
        public static async Task<MemberDetail?> GetMemberAsync(int memberId, string token)
        {
            try
            {
                ...
                var url = $"/api/Member/Get?id={memberId}";
                using var requestMessage = new HttpRequestMessage(HttpMethod.Get, url);
                requestMessage.Headers.Authorization =
                    new System.Net.Http.Headers.AuthenticationHeaderValue("Bearer", token);
                ...
                if (response.IsSuccessStatusCode)
                {
                    return await response.Content.ReadFromJsonAsync(
                        AppJsonContext.Default.MemberDetail, cancellationToken: cts.Token);
                }

                var msg = await TryGetErrorMessage(response);
                ErrorDisplayService.ShowError("Person", msg ?? $"Server returned {(int)response.StatusCode}");
                return null;
            }
            catch (TaskCanceledException)
            {
                ErrorDisplayService.ShowError("Person", "Request timed out while loading the person.");
                return null;
            }
            catch (Exception ex)
            {
                ErrorDisplayService.ShowError("Person", ex);
                return null;
            }
        }

        /// <summary>
        /// Reads a generic { success, message, data:[{value,text}] } dropdown endpoint.
        /// </summary>
        private static async Task<List<DropDownItem>> GetDropDownAsync(string url, string source, string token)
        {
            try
            {
                if (string.IsNullOrEmpty(token))
                ...
                if (response.IsSuccessStatusCode)
                {
                    ...
                    // Preferred shape: { success, message, data: [ ... ] }
                    var wrapper = JsonSerializer.Deserialize(json, AppJsonContext.Default.DropDownResponse);
                    if (wrapper?.Data?.Count > 0)
                        return wrapper.Data;

                    // Fallback: a bare array of { value, text }.
                    var direct = JsonSerializer.Deserialize(json, AppJsonContext.Default.ListDropDownItem);
                    return direct ?? new List<DropDownItem>();
                }

                ErrorDisplayService.ShowError(source, $"Server returned {(int)response.StatusCode}");
                return new List<DropDownItem>();
            }
            catch (Exception ex)
            {
                ErrorDisplayService.ShowError(source, ex);
                return new List<DropDownItem>();
            }
        }

        /// <summary>
        /// Gets the list of titles (Mr, Mrs, ...) for the title dropdown.
        /// Mirrors GET /api/GeneralRead/titledrd.
        /// </summary>
        public static Task<List<DropDownItem>> GetTitlesAsync(string token) =>
            GetDropDownAsync("/api/GeneralRead/titledrd", "Titles", token);

        /// <summary>
        /// Gets the list of entity types (Individual, Company, ...) for the entity-type dropdown.
        /// Mirrors GET /api/GeneralRead/membersearchgridclienttype.
        /// </summary>
        public static Task<List<DropDownItem>> GetEntityTypesAsync(string token) =>
            GetDropDownAsync("/api/GeneralRead/membersearchgridclienttype", "Entity Types", token);

        /// <summary>
        /// Searches for people that can be selected as a leader for the current member.
        /// Mirrors GET /api/GeneralRead/companymemberlistselectableasleader.
        /// </summary>
        public static async Task<List<LeaderItem>> SearchLeadersAsync(string searchPattern, int memberId, string token)
        {
            try
            {
                if (string.IsNullOrEmpty(token))
                    return new List<LeaderItem>();

                var url = $"/api/GeneralRead/companymemberlistselectableasleader" +
                          $"?searchPattern={Uri.EscapeDataString(searchPattern ?? string.Empty)}&memberId={memberId}";
                ...
                if (response.IsSuccessStatusCode)
                {
                    ...
                    // The payload may be a bare array or wrapped in a { data: [...] } envelope.
                    ...
                    if (arrayElement.ValueKind == JsonValueKind.Array)
                    {
                        var list = JsonSerializer.Deserialize(
                            arrayElement.GetRawText(), AppJsonContext.Default.ListLeaderItem);
                        return list ?? new List<LeaderItem>();
                    }

                    return new List<LeaderItem>();
                }

                ErrorDisplayService.ShowError("Leaders", $"Server returned {(int)response.StatusCode}");
                return new List<LeaderItem>();
            }
            catch (Exception ex)
            {
                ErrorDisplayService.ShowError("Leaders", ex);
                return new List<LeaderItem>();
            }
        }

        /// <summary>
        /// Creates or updates a person. Mirrors the web POST /Member/UpdateMemberDetailOnTask.
        /// A <see cref="UpdateMemberRequest.MemberId"/> of 0 indicates a new person.
        /// </summary>
        public static async Task<ApiResult> SaveMemberAsync(UpdateMemberRequest request, string token)
        {
            try
            {
                if (string.IsNullOrEmpty(token))
                ...
                if (Connectivity.Current.NetworkAccess != NetworkAccess.Internet)
                    return ApiResult.Failure("No internet connection. Please try again when you are online.");

                using var requestMessage = new HttpRequestMessage(HttpMethod.Post, "/api/Member/UpdateMemberDetailOnTask");
                requestMessage.Headers.Authorization =
                    new System.Net.Http.Headers.AuthenticationHeaderValue("Bearer", token);
                requestMessage.Content = JsonContent.Create(request, AppJsonContext.Default.UpdateMemberRequest);

                using var cts = new CancellationTokenSource(TimeSpan.FromSeconds(20));
                var response = await HttpClient.SendAsync(requestMessage, cts.Token);

                if (response.IsSuccessStatusCode)
                    return ApiResult.Success("Person saved successfully.");

                var msg = await TryGetErrorMessage(response);
                var error = msg ?? $"Server returned {(int)response.StatusCode}";
                ErrorDisplayService.ShowError("Save Person", error);
                return ApiResult.Failure(error);
            }
            catch (TaskCanceledException)
            {
                ErrorDisplayService.ShowError("Save Person", "Request timed out while saving.");
                return ApiResult.Failure("Request timed out while saving.");
            }
            catch (Exception ex)
            {
                ErrorDisplayService.ShowError("Save Person", ex);
                return ApiResult.Failure(ex.Message);
            }
        }
